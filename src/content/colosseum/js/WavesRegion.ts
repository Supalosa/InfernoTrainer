import { Manticore, Player, Settings } from "osrs-sdk";
...
export class WavesRegion extends ColosseumRegion {
  ...
  override initialiseRegion() {
    const player = new Player(this, { x: 27, y: 29 });
    this.addPlayer(player);
    player.freeze(this.world.getReadyTimer);

    // Keep these unaggressive: this page is initially a visual NPC sandbox.
    this.addMob(new Manticore(this, { x: 21, y: 24 }));
    this.addMob(new Minotaur(this, { x: 29, y: 24 }));
    this.addMob(new SerpentShaman(this, { x: 23, y: 30 }));
    this.addMob(new JavelinColossus(this, { x: 20, y: 30 }));
    this.addMob(new ShockwaveColossus(this, { x: 29, y: 32 }));
